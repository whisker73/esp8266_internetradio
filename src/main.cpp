...
//
// The SPI interface for VS1053 and TFT uses I2C
//

// Wiring:
// NodeMCU  GPIO    Pin to program  Wired to LCD        Wired to VS1053      Wired to rest
// -------  ------  --------------  ---------------     -------------------  ---------------------
// D0       GPIO16  16              -                   pin 5 DCS            -
// D1       GPIO5    5              pin SCLK            -                    -
// D2       GPIO4    4              pin SDA             -                    -
// D3       GPIO0    0 FLASH        -                   -                    Control button "Next station"
// D4       GPIO2    2              -                   pin 3 DREQ                    (OR)Control button "Station 1"
// D5       GPIO14  14 SCLK         -                   pin 9 SCLK            -
// D6       GPIO12  12 MISO         -                   pin 8 MISO           -
// D7       GPIO13  13 MOSI         -                   pin 7 MOSI           -
// D8       GPIO15  15              -                   pin 6 HCS             LED on nodeMCU
// D9       GPI03    3 RXD0         -                   -                    Reserved serial input
// D10      GPIO1    1 TXD0         -                   -                    Reserved serial output
// -------  ------  --------------  ---------------     -------------------  ---------------------
// GND      -        -              pin (GND)         pin GND            Power supply
// VCC 3.3  -        -              pin (3.3V)        pin 1 3.3          LDO 3.3 Volt
// VCC 5 V  -        -              pin               pin 2 5V             Power supply
// RST      -        -              pin               pin 4 RESET          Reset circuit
//
...
void displayvolume()
{
  ...
  display.fillRect ( 0, 126, pos, 2 ,BLACK) ;             // Paint red part
  display.fillRect ( pos, 126, 160 - pos, 2 ,BLACK) ;   // Paint green part
#endif
}
...
void displayinfo ( const char* str, uint16_t pos, uint16_t height, uint16_t color )
{
  ...
  display.setTextColor(WHITE);                  // Prepare to show the info
  display.println ( buf ) ;                         // Show the string
  display.display();
#endif
}
...
void showstreamtitle ( const char *ml, bool full )
{
  ...
  displayinfo ( streamtitle, 20, 40, 1 ) ;   // Show title at position 20
}
...
bool connecttohost()
{
  ...
  displayinfo ( "** Internet radio **", 0, 20, 1 ) ;
  ...
  displayinfo ( pfs, 40, 66, 1 ) ;             // Show info at position 60..125
...
bool connecttofile()
{
  ...
  displayinfo ( "Playing from local file",
                40, 68, 1 ) ;                        // Show Source at position 60
    ...
    else if ( b == '\n' )                              // Linefeed ?
    {
      ...
      if ( chkhdrline ( metaline.c_str() ) )           // Reasonable input?
      {
        ...
        else if ( lcml.startsWith ( "icy-name:" ) )
        {
          ...
          displayinfo ( icyname.c_str(), 40, 68,
                        1 ) ;                     // Show station name at position 60
        }
...
void setup()
{
  ...
  xml.init ( xmlbuffer, sizeof(xmlbuffer), &XML_callback ) ; // Initilize XML stream.
